...
interface RawVisemeEntry {
  time: number;
  viseme: string | null;
}

interface AvatoonModelProps {
  url: string;
  goal: string | null;
...
export function CameraFovAnimator({ readyToPlay }: { readyToPlay: boolean }) {
  ...
  const [targetFov, setTargetFov] = useState(15);
...
export default function Avatoon({
  ...
  return (
    <>
      ...
      <Canvas camera={{ position: [0, 1.5, 2.2], fov: 17 }}>
